(function() {
  ...
  function getDefaultConfig() {
    return {
      ...
      colorboxUrl: 'https://cdn.bootcss.com/jquery.colorbox/1.6.4/jquery.colorbox-min.js',
  ...
  window._shang = function(config) {
    ...
    config.shangUrl = config.baseUrl + '/paycenter/reward/widget?entity=user&id=' + config.uid;
    ...
    $(document).ready(function() {
      ...
      if (container.length > 0 && !config.noHtml) {
        ...
        if (config.showAskAuthorLink) {
          defaultHtml += '  <div class="ask-author">看完了？还不过瘾？点此<a href="' + config.askUrl + '" class="zhi12-popup" data-box-width="680p">付费提问</a></div>';
        }

        // config.text 不为空则显示一行文本
        if (config.text.replace(/(^s*)|(s*$)/g, "").length !== 0) {
          defaultHtml += '  <div class="shang-message">' + plain_text(config.text) + '</div>';
        }
        defaultHtml += '  <div><a class="zhi12-popup zhi12-widget" rel="nofollow" href="{{url}}" target="_blank"><img class="shang-icon" src="' + config.imageUrl + '" alt="打赏" width="' + config.imageWidth + '" height="' + config.imageHeight + '" /></a></div>';
        defaultHtml += '</div>';
  ...
  function init($, window) {
    ...
    (function main() {
      ...
      */
      ...
      // 加载 CSS 文件.
      $('<link>').attr({rel: 'stylesheet', type: 'text/css', href: config.cssUrl}).appendTo('head');
...
return;
    ...
    // 在新窗口打开 zhi12-popup 链接.
    // 避免重复绑定.
    $('.zhi12-popup').unbind('click').bind('click', function(e) {
      ...
      var top = clientHeight > box_inner_height ? (clientHeight - box_inner_height) / 2 : 0;
      var left = clientWidth > box_inner_width ? (clientWidth - box_inner_width) / 2 : 0;

      // 在新窗口打开链接.
      var url = $(this).attr('href');
      var features = 'height=' + box_inner_height + ', width=' + box_inner_width + ', top=' + top + ', left=' + left + ', toolbar=no, menubar=no, scrollbars=no, location=no, status=no';
